...
public class DocumentationGenerator
{
    private class DocAssemblyInfo
    {
        ...
        public SortedDictionary<string, DocNamespaceInfo> Namespaces = [];
    ...
    private class DocGenSession : FileSession
    {
        ...
        public override void CleanUp(HttpResponse response, bool wasModified) => base.CleanUp(response, true);
    ...
    private readonly SortedDictionary<string, DocAssemblyInfo> _assemblies;
    private readonly SortedDictionary<string, DocTypeInfo> _types;
    private readonly SortedDictionary<string, DocMemberInfo> _members;
    private readonly string _usernamePasswordFile;
    private readonly List<string> _assembliesLoaded = [];
    public List<string> AssembliesLoaded => _assembliesLoaded;
    private readonly List<Tuple<string, string>> _assemblyLoadErrors = [];
    public List<Tuple<string, string>> AssemblyLoadErrors => _assemblyLoadErrors;
    ...
    private class DocMemberComparer : IComparer<MemberInfo>
    {
        ...
        public static readonly DocMemberComparer Instance = new();

        private static char getTypeChar(MemberInfo member) => member.MemberType switch
        {
            MemberTypes.Constructor => 'C',
            MemberTypes.Event => 'E',
            MemberTypes.Field => 'F',
            MemberTypes.Method => _operators.Contains(member.Name) ? 'O' : 'M',
    ...
    public DocumentationGenerator(string[] paths, string usernamePasswordFile = null, string copyDllFilesTo = null)
    {
        ...
        foreach (var path in paths)
        {
            ...
            {
                var loadFromFile = copyDllFilesTo != null ? Path.Combine(copyDllFilesTo, f.Name) : f.FullName;
                try
                {
                    var docsFile = f.FullName.Remove(f.FullName.Length - 3) + "xml";
                    var a = Assembly.LoadFile(loadFromFile);
                    var e = XElement.Load(docsFile);
                    ...
                    foreach (var t in a.GetExportedTypes().Where(shouldTypeBeDisplayed))
                    {
                        var typeFullName = getTypeFullName(t);
                        var namespc = t.Namespace ?? NoNamespaceName;
                        var doc = e.Element("members").Elements("member").FirstOrDefault(n => n.Attribute("name").Value == typeFullName);
                        ...
                        var typeinfo = new DocTypeInfo { Type = t, Documentation = doc, Members = [] };
    ...
    private static string getTypeFullName(Type t) => "T:" + (t.IsGenericType ? t.GetGenericTypeDefinition() : t).FullName.TrimEnd('&').Replace("+", ".");

    private FileAuthenticator _authenticator;
    private UrlResolver _resolver;
    private readonly KeyValuePair<string, string>[] _csses = typeof(Css)
        .GetFields(BindingFlags.Static | BindingFlags.Public)
        .Where(f => f.FieldType == typeof(string))
        .Select(f => Ut.KeyValuePair(f.Name, (string) f.GetValue(null)))
        .ToArray();
    ...
    private HttpResponse handle(HttpRequest req) =>
        req.Url.Path == ""
            ...
            : Session.EnableManual<DocGenSession>(req, session =>
            {
                ...
                var token = req.Url.Path.Substring(1).UrlUnescape();
                var tokens = token.Split('/');

                var status = HttpStatusCode._200_OK;
                string title;
                object content;
                ...
                else if (tokens.Length == 2 && _assemblies.TryGetValue(tokens[0], out var docAssembly) && docAssembly.Namespaces.ContainsKey(tokens[1]))
                {
                    asm = tokens[0];
                    ns = tokens[1];
                    title = "Namespace: " + ns;
                ...
                else if (_types.TryGetValue(token, out var value))
                {
                    type = value.Type;
                    ...
                    content = generateTypeDocumentation(req, value.Type, value.Documentation);
                }
                else if (_members.TryGetValue(token, out var doc))
                {
                    member = doc.Member;
                    ...
                    title = getMemberTitle(member) + ": " + stringSoup(
                        ...
                        member.Name);
                    content = generateMemberDocumentation(req, doc.Member, doc.Documentation);
                }
                                ...
                                new TD { class_ = "left" }._(
                                    ...
                                    new DIV { class_ = "boxy legend" }._(
                                        ...
                                        new TABLE { class_ = "legend" }._(
                                            new TR(
                                                new string[][] { ["Class", "Struct", "Enum", "Interface", "Delegate"], ["Constructor", "Method", "Property", "Event", "Field"] }
    ...
    private IEnumerable<object> generateAllTypes(HttpRequest req)
    {
        ...
        yield return new DIV { class_ = "innercontent" }._(generateAll(
            ...
            ["Static classes", "Classes", "Structs", "Enums", "Interfaces", "Delegates"],
    ...
    private IEnumerable<object> generateAllMembers(HttpRequest req)
    {
        ...
        yield return new DIV { class_ = "innercontent" }._(generateAll(
            ...
            ["Constructors", "Methods", "Properties", "Events", "Fields"],
    ...
    private static IEnumerable<object> generateAll<TSource>(IEnumerable<TSource> source, Func<TSource, int> categorize,
    ...
    private IEnumerable<object> friendlyTypeName(Type t, bool includeNamespaces = false, bool includeOuterTypes = false, bool variance = false, IHttpUrl baseUrl = null, bool inclRef = false, bool isOut = false, bool isThis = false, bool isParams = false, bool span = false, bool modifiers = false, bool namespaceSpan = false, Dictionary<Type, Type> subst = null, bool wbrs = false)
    {
        ...
        else
        {
            ...
            else if (includeNamespaces && !t.IsGenericParameter)
            {
                ...
                yield return namespaceSpan ? new SPAN { class_ = "namespace" }._(arr) : arr;
            }
            ...
            if (hasGenericTypeParameters)
            {
                ...
                var skip = t.IsNested ? t.DeclaringType.GetGenericArguments().Length : 0;
    ...
    private object friendlyMemberName(MemberInfo m, bool returnType = false, bool containingType = false, bool parameterTypes = false, bool parameterNames = false, bool parameterDefaultValues = false, bool omitGenericTypeParameters = false, bool namespaces = false, bool variance = false, bool indent = false, string url = null, IHttpUrl baseUrl = null, bool stringOnly = false, bool modifiers = false, Dictionary<Type, Type> subst = null, bool wbrs = false)
    {
        ...
        if (m.MemberType is MemberTypes.Constructor or MemberTypes.Method)
        {
            ...
            return stringOnly ? f : new SPAN { class_ = m.MemberType.ToString(), title = stringSoup(friendlyMemberName(m, true, true, true, true, true, stringOnly: true, modifiers: true)) }._(f);
        }

        var arr = Ut.NewArray<object>(
            returnType && m.MemberType == MemberTypes.Property ? [friendlyTypeName(((PropertyInfo) m).PropertyType, includeNamespaces: namespaces, includeOuterTypes: true, baseUrl: baseUrl, span: !stringOnly, subst: subst), " "] :
            returnType && m.MemberType == MemberTypes.Event ? [friendlyTypeName(((EventInfo) m).EventHandlerType, includeNamespaces: namespaces, includeOuterTypes: true, baseUrl: baseUrl, span: !stringOnly, subst: subst), " "] :
            returnType && m.MemberType == MemberTypes.Field ? [friendlyTypeName(((FieldInfo) m).FieldType, includeNamespaces: namespaces, includeOuterTypes: true, baseUrl: baseUrl, span: !stringOnly, subst: subst), " "] : (object[]) null,
    ...
    private IEnumerable<object> friendlyMethodName(MethodBase m, bool returnType = false, bool containingType = false, bool parameterTypes = false, bool parameterNames = false, bool parameterDefaultValues = false, bool omitGenericTypeParameters = false, bool includeNamespaces = false, bool variance = false, bool indent = false, string url = null, IHttpUrl baseUrl = null, bool isDelegate = false, bool stringOnly = false, bool modifiers = false, Dictionary<Type, Type> subst = null, bool wbrs = false)
    {
        ...
        if (returnType && m.MemberType != MemberTypes.Constructor)
        {
            ...
            if (meth.Name is not "op_Implicit" and not "op_Explicit")
            {
        ...
        if ((m.MemberType == MemberTypes.Constructor || isDelegate) && url != null)
            yield return stringOnly ? friendlyTypeName(m.DeclaringType, includeNamespaces, includeOuterTypes: isDelegate, span: false, subst: subst) : new STRONG { class_ = "member-name" }._(new A { href = url }._(friendlyTypeName(m.DeclaringType, includeNamespaces, includeOuterTypes: isDelegate, span: true, subst: subst, wbrs: wbrs)));
        else if (isDelegate)
            yield return stringOnly ? friendlyTypeName(m.DeclaringType, includeNamespaces, includeOuterTypes: true, variance: variance, subst: subst) : new STRONG { class_ = "member-name" }._(friendlyTypeName(m.DeclaringType, includeNamespaces, includeOuterTypes: true, variance: variance, baseUrl: baseUrl, span: true, subst: subst, wbrs: wbrs));
            ...
            yield return stringOnly ? friendlyTypeName(m.DeclaringType, includeNamespaces, subst: subst) : new STRONG { class_ = "member-name" }._(friendlyTypeName(m.DeclaringType, includeNamespaces, baseUrl: baseUrl, span: true, subst: subst, wbrs: wbrs));
        if (!stringOnly && !indent && (m.IsGenericMethod || (parameterNames && m.GetParameters().Length > 0))) yield return new WBR();
        if (m.MemberType != MemberTypes.Constructor && !isDelegate)
        {
            if (containingType) yield return ".";
            var f = cSharpCompatibleMethodName(m.Name, friendlyTypeName(((MethodInfo) m).ReturnType, includeOuterTypes: true, span: !stringOnly, subst: subst, wbrs: wbrs));
        ...
        {
            yield return indent && m.GetParameters().Length > 0 ? "(\n    " : "(";
            if (!stringOnly && !indent && m.GetParameters().Length > 0) yield return new WBR();
            var first = true;
            foreach (var p in m.GetParameters())
            {
                ...
                first = false;
                yield return stringOnly ? stringSoup(f) : new SPAN { class_ = "parameter" }._(f);
            }
            yield return indent && m.GetParameters().Length > 0 ? "\n)" : ")";
        }
    }
    ...
    private static readonly string[] _operators = Ut.NewArray(
    ...
    private static object cSharpCompatibleMethodName(string methodName, object returnType = null, bool wbrs = false) => methodName switch
    {
        "op_Implicit" => returnType == null ? "implicit operator" : new object[] { "implicit operator ", returnType },
        "op_Explicit" => returnType == null ? "explicit operator" : new object[] { "explicit operator ", returnType },
        "op_UnaryPlus" or "op_Addition" => "operator+",
        "op_UnaryNegation" or "op_Subtraction" => "operator-",
        ...
        _ => wbrs ? addWbrs(methodName) : methodName,
    };
    ...
    private IEnumerable<object> friendlyPropertyName(PropertyInfo property, bool parameterTypes, bool parameterNames, bool includeNamespaces, bool indent, string url, IHttpUrl baseUrl, bool stringOnly, Dictionary<Type, Type> subst, bool wbrs)
    {
        ...
        if (prms.Length > 0)
        {
            yield return stringOnly ? "this" : new STRONG { class_ = "member-name" }._(url == null ? "this" : new A { href = url }._("this"));
            yield return indent ? "[\n    " : "[";
            ...
            foreach (var p in prms)
            {
                ...
                var arr = Ut.NewArray<object>(
                    ...
                    parameterTypes && parameterNames ? " " : null,
                    parameterNames ? (stringOnly ? p.Name : new STRONG(p.Name)) : null
                );
                yield return stringOnly ? arr : new SPAN { class_ = "parameter" }._(arr);
            }
            yield return indent ? "\n]" : "]";
        }
        else
            yield return stringOnly ? property.Name : new STRONG { class_ = "member-name" }._(url == null ? property.Name : new A { href = url }._(global::RT.DocGen.DocumentationGenerator.addWbrs(property.Name)));
    }

    private static object addWbrs(string name) => name.RegexMatches(@"[\p{Ll}\p{Lm}\p{Lo}\p{N}\p{Pc}]+|[\p{Lu}\p{Lt}]+[\p{Ll}\p{Lm}\p{Lo}\p{N}\p{Pc}]*").Select(m => m.Value).InsertBetween<object>(new WBR());

    private bool shouldMemberBeDisplayed(MemberInfo m) => m switch
    {
        { ReflectedType.IsEnum: true, Name: "value__" } => false,
        ConstructorInfo cInfo => !cInfo.IsPrivate,
        MethodInfo mInfo => !mInfo.IsPrivate && !isPropertyGetterOrSetter(m) && !isEventAdderOrRemover(m),
        EventInfo eInfo => (eInfo.GetAddMethod() != null && !eInfo.GetAddMethod().IsPrivate) || (eInfo.GetRemoveMethod() != null && !eInfo.GetRemoveMethod().IsPrivate),
        PropertyInfo pInfo => (pInfo.GetGetMethod() != null && !pInfo.GetGetMethod().IsPrivate) || (pInfo.GetSetMethod() != null && !pInfo.GetSetMethod().IsPrivate),
        FieldInfo fInfo => !fInfo.IsPrivate && !isEventField(m),
        Type { MemberType: MemberTypes.NestedType } tInfo => shouldTypeBeDisplayed(tInfo),
        _ => false
    };

    private bool shouldTypeBeDisplayed(Type t) => !(t.IsNested && t.IsNestedPrivate) && (t.FullName != "XamlGeneratedNamespace.GeneratedInternalTypeHelper");

    private static string documentationCompatibleMemberName(MemberInfo m, Type reflectedType = null, Dictionary<Type, Type> genericSubstitutionsForHidingDetection = null)
    {
        m = findMemberDefinition(m);
        var sb = new StringBuilder();
        if (m.MemberType is MemberTypes.Method or MemberTypes.Constructor)
        {
            var mi = m as MethodBase;
            sb.Append("M:");
        ...
        else if (m.MemberType is MemberTypes.Field or MemberTypes.Event)
        {
        ...
        if (m is MethodInfo { Name: "op_Implicit" or "op_Explicit" } mth)
            sb.Append("~" + stringifyParameterType(mth.ReturnType, mth.ReflectedType, mth, genericSubstitutionsForHidingDetection));
        return sb.ToString();
    }

    private static void appendParameterTypes(StringBuilder sb, ParameterInfo[] parameters, Type type, MethodInfo method = null, Dictionary<Type, Type> genericSubstitutions = null)
    {
        var first = true;
    ...
    private static string stringifyParameterType(Type parameterType, Type type, MethodInfo method, Dictionary<Type, Type> genericSubstitutions)
    {
        ...
        {
            var fullName = parameterType.GetGenericTypeDefinition().FullName.Replace("+", ".");
            var constructName = "";
            var genericArguments = parameterType.GetGenericArguments().AsEnumerable();
            while (fullName.RegexMatch(@"`(\d+)", out var m))
            {
    ...
    private static bool isPropertyGetterOrSetter(MemberInfo member)
    {
        if (member.MemberType != MemberTypes.Method)
            return false;
        if (!member.Name.StartsWith("get_") && !member.Name.StartsWith("set_"))
            return false;
        var partName = member.Name.Substring(4);
    ...
    private static bool isEventAdderOrRemover(MemberInfo member)
    {
        if (member.MemberType != MemberTypes.Method)
            return false;
        if (!member.Name.StartsWith("add_") && !member.Name.StartsWith("remove_"))
            return false;
        var partName = member.Name.Substring(member.Name.StartsWith("add_") ? 4 : 7);
        return member.DeclaringType.GetMembers().Any(m => m.MemberType == MemberTypes.Event && m.Name == partName);
    }

    private static bool isEventField(MemberInfo member) => member is FieldInfo && member.DeclaringType.GetMembers().Any(m => m.MemberType == MemberTypes.Event && m.Name == member.Name);
    ...
    private object generateTypeBullet(string typeFullName, MemberInfo selectedMember, HttpRequest req)
    {
        ...
        var cssClass = typeinfo.TypeCssClass;
            ...
            selectedMember != null && !typeof(Delegate).IsAssignableFrom(typeinfo.Type) && isMemberInside(selectedMember, typeinfo.Type)
                ? new UL(typeinfo.Members.Where(mkvp => isPublic(mkvp.Value.Member))
                    ...
                    .Select(mkvp =>
                    {
                        ...
                        var css = mkvp.Value.Member.MemberType.ToString();
    ...
    private static bool isMemberInside(MemberInfo member, Type containingType) => isNestedTypeOf(member is Type type ? type : member.DeclaringType, containingType);

    private static bool isNestedTypeOf(Type nestedType, Type containingType) => nestedType == containingType || (nestedType.IsNested && isNestedTypeOf(nestedType.DeclaringType, containingType));
        ...
        yield return new DIV { class_ = "innercontent" }._(
            ...
            new TABLE { class_ = "doclist" }._(
                ns.Types.Where(t => !t.Value.Type.IsNested).Select(kvp => new TR(
                    ...
                    new TD(
                        kvp.Value.Documentation?.Elements("image").Select(image => interpretImage(image.Attribute("type")?.Value, image.Value)),
    ...
    private static object interpretImage(string type, string content) => type == "raw" ? new DIV { class_ = "image" }._(new RawTag(content)) : null;
    ...
    private static string getMemberTitle(MemberInfo member)
    {
        var isStatic =
    ...
    private IEnumerable<object> generateTypeDocumentationInner(HttpRequest req, Type type, XElement documentation)
    {
        ...
        else
        {
            ...
            if (isDelegate)
            {
                if (delegateInvokeMethod.GetParameters().Length > 0)
    ...
    private sealed class DocHideInfo(MemberInfo member, string memberName, Dictionary<Type, Type> substitutions)
    {
        public MemberInfo Member { get; private set; } = member;
        public string MemberName { get; private set; } = memberName;
        public Dictionary<Type, Type> Substitutions { get; private set; } = substitutions;
    }
    ...
    private IEnumerable<object> generateParameterDocumentation(HttpRequest req, MethodBase method, XElement methodDocumentation)
    {
        ...
        yield return new TABLE { class_ = "doclist" }._(
            parameterInfos.Select((pi, index) => new TR(
                new TD { class_ = "membertype" }._(
    ...
    private static bool summaryIsMergable(XElement doc1, XElement doc2)
    {
        if (doc1 == null)
            return doc2 == null;
        else if (doc2 == null)
        ...
        return (doc1.Element("remarks") == null) == (doc2.Element("remarks") == null) && XNode.DeepEquals(doc1.Element("summary"), doc2.Element("summary"));
    }
    ...
    private IEnumerable<object> formatMemberExtraInfoItems(HttpRequest req, MemberInfo member, bool markInterfaceMethods, Type inheritedFrom, Dictionary<Type, Type> subst, DocHideInfo[] hides)
    {
        ...
        var showVirtual = method.IsVirtual;
        foreach (var baseMember in getImplementedMembers(member))
        {
            if (baseMember.DeclaringType.IsInterface)
            {
                ...
                var url = _members.ContainsKey(dcmn) ? req.Url.WithPath("/" + dcmn.UrlEscape()).ToHref() : null;
            ...
            else
            {
                var dcmn = documentationCompatibleMemberName(baseMember);
                var url = _members.ContainsKey(dcmn) ? req.Url.WithPath("/" + dcmn.UrlEscape()).ToHref() : null;
                yield return new LI("Overrides: ", friendlyMemberName(baseMember, containingType: true, parameterTypes: true, url: url, baseUrl: req.Url));
                if (method.IsFinal)
    ...
    private static MemberInfo findMemberDefinition(MemberInfo member)
    {
    ...
    private static IEnumerable<MemberInfo> getImplementedMembers(MemberInfo member)
    {
        var method =
            member is MethodInfo methodInfo ? methodInfo :
            member is PropertyInfo pi ? (pi.GetGetMethod() ?? pi.GetSetMethod()) :
    ...
    private static bool sameExcept(MethodInfo m1, MethodInfo m2, Type[] genericTypeParameters, Type[] genericTypeArguments)
    {
        if (m1.Name != m2.Name)
            return false;
    ...
    private static bool sameExcept(Type t1, Type t2, Type[] genericTypeParameters, Type[] genericTypeArguments)
    {
        if (t1 == t2)
            return true;
    ...
    private LI implementsInterfaces(Type type, HttpRequest req) =>
        type.GetInterfaces() is { Length: > 0 } infs
            ? new LI(
    ...
    private LI implementedBy(Type type, HttpRequest req) =>
        _types.Select(kvp => kvp.Value.Type).Where(t => t.GetInterfaces().Any(i => i.Equals(type) || (i.IsGenericType && i.GetGenericTypeDefinition().Equals(type)))).ToArray() is { Length: > 0 } implementedBy
            ? new LI(
    ...
    private object interpretNodes(IEnumerable<XNode> nodes, HttpRequest req) => nodes.Select(n => interpretNode(n, req));
    ...
    {
        if (_types.TryGetValue(token, out var tDoc))
            return friendlyTypeName(tDoc.Type, includeNamespaces, includeOuterTypes: true, baseUrl: req.Url, span: true);
        else if (_members.TryGetValue(token, out var mDoc))
            return friendlyMemberName(mDoc.Member, containingType: true, parameterTypes: true, namespaces: includeNamespaces, url: req.Url.WithPath("/" + token.UrlEscape()).ToHref(), baseUrl: req.Url);
    ...
    private static object foreignTypeName(Type type, bool includeNamespaces) => new SPAN { class_ = "type" }._(foreignTypeNameInner(type, includeNamespaces));
    private static object foreignTypeName(string type, bool includeNamespaces) => new SPAN { class_ = "type" }._(foreignTypeNameInner(type, includeNamespaces));

    private static IEnumerable<object> foreignTypeNameInner(Type type, bool includeNamespaces)
    {
        ...
        yield return new STRONG(type.Name.RegexReplace("`\\d+", string.Empty));
        if (type.IsGenericType)
        {
            yield return "<";
    ...
    private static IEnumerable<object> foreignTypeNameInner(string type, bool includeNamespaces)
    {
        var numGenerics = 0;
    ...
    private IEnumerable<object> interpretPre(XElement elem, HttpRequest req)
    {
        ...
        var commonIndentation = lines.Min(lin =>
        {
            ...
            return lin.First() is string first ? (first == "" ? null : first.TakeWhile(c => c == ' ').Count()) : 0;
        });
    ...
    private static string stringSoup(object obj)
    {
        var sb = new StringBuilder();
        stringSoup(obj, sb);
        return sb.ToString();
    ...
    private static void stringSoup(object obj, StringBuilder sb)
    {
        switch (obj)
        {
            case null:
            ...
            case IEnumerable enumerable:
                ...
                break;
            default:
                throw new InvalidOperationException($@"Encountered {obj.GetType().FullName} where only strings are expected.");
        }
    }
